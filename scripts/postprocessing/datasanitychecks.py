...
class DataSanityCheck: 
    ...
    - check if we have a reverse ordering in z-direction
    - check valid range 
    """
    ...
    def __init__(self, 
        ...
        self.z = zspacing 
        self.smoothing_sigma = smoothing_sigma # in mm
    ...
    def is_reverse_zordering(self): 
        if self.a < 0: return 1
        return 0 
    
    def is_valid_zspacing(self): 
        if ((self.zhat - 3*self.zhat_std) < self.z) and (self.z < (self.zhat + 3*self.zhat_std)): 
            return 1 
        return 0 
    ...
    def get_index_bounds(self, 
        ...
        if (np.min(scores) > self.lower_bound_score): 
            ...
            min_invalid_indices = negative_slope_indices[np.where(negative_slope_indices <= min_valid_index)]
            if len(min_invalid_indices) > 0: 
                min_valid_index = max(min_invalid_indices)
            else: min_valid_index = 0
        ...
        if (np.max(scores) < self.upper_bound_score): 
            ...
            max_invalid_indices = negative_slope_indices[np.where(negative_slope_indices >= max_valid_index)]
            if len(max_invalid_indices) > 0: 
                max_valid_index = min(max_invalid_indices)
            else: max_valid_index = len(scores) 
        
        return min_valid_index, max_valid_index
    
    
    def remove_invalid_regions(self, scores): 
        smoothed_scores = gaussian_filter(scores, 
                                          sigma=self.smoothing_sigma/self.z)


        diff = np.array(smoothed_scores[1:]) - np.array(smoothed_scores[:-1])
        negative_slope_indices = np.where(diff < 0)[0]
        
        min_valid_index, max_valid_index = self.get_index_bounds(negative_slope_indices, 
                                                                 smoothed_scores)
