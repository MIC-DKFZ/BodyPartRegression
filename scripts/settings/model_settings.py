...
class ModelSettings:
    ...
    def get_custom_transform(self):
        ...
        for transform in [GaussNoise, ShiftHU, ScaleHU, AddFrame]:
            if not transform in self.transform_params: continue
            params = self.transform_params[transform.__name__]
    ...
    def save(self, save_path=""):
        for key in ["albumentation_transform", "custom_transform", "filepath", "json_dict"]: 
    ...
    def load_pickle(self, path):
        ...
        params = {}
        delete_keys = ["square_frame", "circle_frame"]

        for transform in self.custom_transform.__dict__["transforms"]:
            name = type(transform).__name__
            transform_params = transform.__dict__
            params[name] = transform_params
            for key in delete_keys: 
                if key in params[name].keys(): del params[name][key] 

        for transform in self.albumentation_transform:
            name = type(transform).__name__
            transform_params = transform.__dict__
            params[name] = transform_params
    ...
    def __str__(self):
        start_line = "\nBODY PART REGRESSION SETTINGS\n"
        line =  "*******************************************************\n"
        part1_keys = ["model_name", "name", "df_data_source_path", "data_path", "landmark_path", "save_dir", 
        "shuffle_train_dataloader", "random_seed", "deterministic", "save_model", "base_model"]
        part2_keys = ["batch_size", "effective_batch_size", "equidistance_range", "num_slices"]
        part3_keys = ["alpha_h", "beta_h", "loss_order", "lambda_", "alpha", "lr", "epochs"]
