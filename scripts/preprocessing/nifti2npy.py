...
class Nifti2Npy: 
    def __init__(self, 
                 target_pixel_spacing=3.5, 
                 min_hu=-1000,
                 max_hu=1500,
                 ipath="/home/AD/s429r/Documents/Data/DataSet/Images/", 
                 opath="/home/AD/s429r/Documents/Data/DataSet/Arrays-3.5mm/",
                 size=128,
                 skip_slices=30,
                 corrupted_files=[],
                 reverse_zaxis=[],
                 sigma=(0.8, 0.8, 0),
                 reference_downscaling_factor=0.25): 
        ...
        self.max_hu = max_hu
        self.size = size
        self.padding = A.PadIfNeeded(min_height=size, min_width=size, always_apply=True, border_mode=cv2.BORDER_CONSTANT)
    ...
    def reorder_volume(self, x, pixel_spacings, affine, filename): 
        ...
        # check z-axis 
        if (np.sign(affine[:, 2][axis_ordering == 2])[0] == -1) or (filename.startswith(tuple(self.reverse_zaxis))): 
            x = np.flip(x)
    ...
    def resize_volume(self, x, pixel_spacings): 
        # TODO: Add gaussian blur before downsampling
    ...
    def dataframe_template(self, filepaths): 
        ...
        df = pd.DataFrame(index=filenames, columns=["nii2npy", "x0", "y0", "z0", "x", "y", "z", "min_x", "max_x", 
                                                     "pixel_spacingx", "pixel_spacingy", "pixel_spacingz"])
        df["nii2npy"] = 0
    ...
    def resize_xy(self, x, pixel_spacings): 
        ...
        sigma = (self.sigma[0] * self.reference_downscaling_factor/downscaling_factor_x, 
    ...
    def add_info2df(self, df, filename, x, pixel_spacings): 
        df.loc[filename, ["pixel_spacingx", "pixel_spacingy", "pixel_spacingz"]] = pixel_spacings
    ...
    def load_volume(self, filepath): 
        ...
        try: x = img_nii.get_fdata(dtype=np.float32)
        except EOFError: 
    ...
    def convert_file(self, filepath, save=True):
        filename = filepath.split("/")[-1]
        ofilepath = self.opath + filename.replace(".nii", "").replace(".gz", "") + ".npy"

        x0, pixel_spacings0, affine = self.load_volume(filepath)
        if not isinstance(x0, np.ndarray): return None, None, None
        x, pixel_spacings = self.reorder_volume(x0, pixel_spacings0, affine, filename)

        check = self.test_pixelspacing(pixel_spacings)
        if check == 1: return None, None, None
        if (x.shape[0] < self.skip_slices) or (x.shape[1] < self.skip_slices) or (x.shape[2] < self.skip_slices): print(f"Not enough slices {x.shape}. Skip file."); return None, None, None
        if len(x.shape) > 3: print(f"Unknown dimensions {x.shape}. Skip file."); return None, None, None
        ...
        if save: np.save(ofilepath, x.astype(np.float32))
        return x, x0, pixel_spacings


    def convert(self, filepaths): 
        df = self.dataframe_template(filepaths)

        for filepath in tqdm(filepaths): 
            filename = filepath.split("/")[-1]
            if filepath in self.corrputed_files: continue
            x, x0, pixel_spacings = self.convert_file(filepath)
