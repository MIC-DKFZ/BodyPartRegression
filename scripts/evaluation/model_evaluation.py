...
class Evaluation: 
    def __init__(self, fontsize=16): 
        self.landmark_names =["pelvis-start", "pelvis-end", "kidney", "lung-start", 
                              "liver-end", "lung-end", "teeth", "nose", "eyes-end"]
        self.landmarkToClassMapping = {0: 0, 1: 1, 2: 2, 3: 2, 4: 2, 5: 3, 6: 4, 7: 5, 8: 5}
        self.colors = ["black", "gray", "silver", "lightcoral", "brown", "chocolate", 
                        "goldenrod", "red", "lightgreen", "green",  
                        "deepskyblue", "steelblue",  "mediumorchid", "plum", "purple", "pink"]
        self.fontsize = fontsize
    def plot_landmarks(self, val_landmark_preds, 
                       min_value=-20, 
                       max_value=20,
                       save_path=False, bin_width=0.5, title=None): 
        _, ax = plt.subplots(figsize=(18, 12))
        for landmark, values in val_landmark_preds.items(): 
            if len(values) <= 1: continue
            x = plt.hist(values, density=True, label=self.landmark_names[landmark], 
                     bins=np.arange(min(values), max(values) + bin_width, bin_width), alpha=0.75)
            plt.annotate(self.landmark_names[landmark], 
        ...
        self.set_scientific_style(ax, legend=False, minor_xticks=0.25, major_xticks=1, minor_yticks=0.1, major_yticks=0.5)
        ...
        if save_path: plt.savefig(save_path + "landmark-prediction-historgram.png", dpi=300, transparent=False, bbox_inches='tight')

    def get_landmark_prediction(self, dataset, dataset_preds):
        """
        ...
        for i, myDict in dataset.landmarks.items():
            ...
            if index not in dataset_preds.keys(): continue
            ys = dataset_preds[index]
            for landmark in myDict["defined_landmarks_i"]: 
                lm_idx = myDict["slice_indices"][np.where(myDict["defined_landmarks_i"] == landmark)[0][0]]
                lm_y = ys[lm_idx]
    ...
    def plot_similar_images(self, dataset, preds, min_value=-20, max_value=20, save_path=False, skip_diff=0.25): 
        targets = np.linspace(min_value, max_value, 5, dtype=int) 

        for target in targets: 
            print(f"Plot corresponding images to the target score {target}")
            if save_path: save_path2 = save_path + f"similar-images-to-score-{target}.png"
            else: save_path2 = False
            self.plot_closest_slices(dataset, preds, target, skip_diff=skip_diff, save_path=save_path2) 
    ...
    def plot_closest_slices(self, dataset, predict_dict, y_target, skip_idx=[], skip_diff=3, save_path=False):
        ...
        for vol_idx in vol_indices: 
            ...
            closest_slice, closest_value, _ = self.get_closest_slice(dataset, predict_dict, y_target, vol_idx)
    ...
    def set_scientific_style(self, ax, legend_anchor=(0.2, 0.98), 
                            minor_xticks=5, major_xticks=20, minor_yticks=0.5, major_yticks=2, legend=True, labelsize=12): 
        plt.rc('font', family='serif')
        plt.grid(True)
        if legend: plt.legend(bbox_to_anchor=legend_anchor, loc=1, frameon=False,  fontsize=12)
    ...
    def get_landmark_prediction_summary(self, dataset, model):
        ...
        results = {i: {"lm": np.round(landmark_vars[i]/total_var, 4), 
                       "mean": np.round(landmark_predictions[i],3), 
                       "var": np.round(landmark_vars[i],3)} for i in range(len(landmark_vars))}
        return results 
    ...
    def landmark_dict_summary(self, landmark_predictions): 
        ...
        myDict = {key: {"lm": np.round(np.var(landmark_predictions[key])/total_var,4),
                        "mean": np.round(np.mean(landmark_predictions[key]), 2), 
                        "var": np.round(np.var(landmark_predictions[key]), 2)} 
                  for key in landmark_predictions.keys() if not len(landmark_predictions[key]) == 0}  
    ...
    def mean_relative_deviation(self, landmark_preds, reference_results): 
        ...
        for key in landmark_preds.keys(): 
            if not key in reference_results.keys(): continue
            landmark_deviations = ((np.array(landmark_preds[key]) - reference_results[key]["mean"])/(max_value - min_value))**2
    ...
    def accuracy(self, dataset, predictions, reference_results, reverse=False): 
        ...
        for key, landmark_dict in dataset.landmarks.items(): 
            obs_classes, pred_classes, scores, accuracy, _ = self.classify_volume(landmark_dict, 
                                                                                  predictions, 
                                                                                  reference_results, 
                                                                                  reverse=reverse)
    ...
    def predict_dataset(self, dataset, model): 
        ...
        for i in tqdm(indices): 
            ...
            with torch.no_grad(): 
                model.eval()
                model.to("cuda")
                try: ys = model(volume.cuda())
                except: print("Out of memory", volume.shape); continue
                preds[i] = [y.item() for y in ys]
        return preds, zs
        
    def classify_volume(self, landmark_dict, predictions, reference_results, reverse=False): 
        vol_idx = landmark_dict["dataset_index"]
        if not vol_idx in predictions.keys(): return [],[], [], 0, None
        analyzed_indices, obs_classes = self.observed_classes(landmark_dict)
        scores = np.array(predictions[vol_idx])[analyzed_indices]
        pred_classes = np.array([self.score_to_class(y, reference_results, reverse=reverse) for y in scores]) 
    ...
    def observed_classes(self, myDict): 
        analyzed_indices = np.arange(min(myDict["slice_indices"]), max(myDict["slice_indices"]))
        ...
        for landmark in myDict['defined_landmarks_i']: 
            ...
            if (stop_slice - start_slice) < 0: continue
    ...
    def score_to_class(self, score, results, reverse): 
        if not reverse: 
            ...
            if 5 in results.keys(): 
                ...
                if (score >= results[4]["mean"]): return 4 # neck 
            ...
            if 5 in results.keys(): 
                ...
                if (score <= results[4]["mean"]): return 4 # neck
    def plot_slope_histogramm(self, filename="slope-histogramm-2.png", bin_width=0.00025): 
        ...
        self.set_scientific_style(ax, major_xticks=0.005, minor_xticks=0.0025, labelsize=14)
        plt.yscale("log")
...
class ModelEvaluation(Evaluation, LookUpTable): 
    ...
    def __init__(self, base_filepath, setup=True, val_dataset=False): 
        ...
        # setup trainer 
        self.trainer = pl.Trainer(gpus=1, max_epochs=self.config["epochs"], precision=16, deterministic=self.config["deterministic"])
        ...
        self.val_landmark_preds, self.val_landmark_preds_ids = self.get_landmark_prediction(self.val_dataset, 
                                                                                            self.val_preds)

        self.val_acc, self.val_std = self.accuracy(self.val_dataset, self.val_preds, self.train_lm_summary)
        self.rmse, self.rmse_std = self.mean_relative_deviation(self.val_landmark_preds, self.train_lm_summary)

        self.min_value = min([self.train_lm_summary[key]["mean"] for key in self.train_lm_summary.keys()])
        self.max_value = max([self.train_lm_summary[key]["mean"] for key in self.train_lm_summary.keys()])
    ...
    def _setup_data(self, val_dataset=False):
        ...
        if val_dataset: 
            ...
            self.train_dataset, _, self.test_dataset = get_datasets(config, 
                                                                    df_data)
        else: 
            self.train_dataset, self.val_dataset, self.test_dataset = get_datasets(config, 
                                                                                   df_data)
    ...
    def get_landmark_xy(self, dataset, preds, landmark):
        ...
        for i, myDict in dataset.landmarks.items():
            index = myDict["dataset_index"]
            if not index in preds.keys(): continue

            ys = preds[index]
            if not landmark in myDict["defined_landmarks_i"]: continue
            x = myDict["slice_indices"][np.where(myDict["defined_landmarks_i"] == landmark)[0][0]]
            y = ys[x]
    ...
    def plot_score2index_xyfix(self, dataset, preds, zs, landmark_anchor=3, ids=np.arange(0,20), save_path=False): 
        landmark_xy = self.get_landmark_xy(dataset, preds, 3)

        plt.figure(figsize=(12, 8))    
        for i in ids: 
            y = preds[i]
            if i not in landmark_xy.keys(): continue
        ...
        plt.title(f"Slice scores for volumes in validation dataset\nAnchor: landmark {landmark_anchor} (0, 0)\n", fontsize=16)
        if save_path: plt.savefig(save_path + f"slice-scores-{landmark_anchor}-anchor-xy-00.png", dpi=300, transparent=False, bbox_inches='tight')
        plt.show()
    ...
    def plot_score2index_xfix(self, dataset, preds, zs, landmark_anchor=3, 
                              ids=np.arange(0, 20), save_path=False, title=None, colors=[]): 
        landmark_xy = self.get_landmark_xy(dataset, preds, landmark_anchor)
        plotted_ids = []
        if len(colors) == 0: colors = self.colors
        fig, ax = plt.subplots(figsize=(12, 8))    
        for i, idx in enumerate(ids): 
            y = preds[idx]
            color = colors[i%len(colors)]
            if idx not in landmark_xy.keys(): continue
        ...
        if save_path: plt.savefig(save_path + f"slice-scores-{landmark_anchor}-anchor-x-0.png", dpi=300, transparent=False, bbox_inches='tight')
        plt.show()

        return plotted_ids

    def plot_score2index(self, dataset, preds, zs, ids=np.arange(0,20), save_path=False, colors=[], title=None): 
        landmark_xy = self.get_landmark_xy(dataset, preds, 3)
        if len(colors) == 0: colors = self.colors
        ...
        plt.ylabel("slice score", fontsize=16)
        plt.title(title, fontsize=16)
        if save_path: plt.savefig(save_path + f"slice-scores.png",  dpi=300, transparent=False, bbox_inches='tight')
        plt.show()
    ...
    def landmarks2score(self, i, dataset, train_results): 
        ...
        expected_scores = [train_results[key]["mean"] for key in train_results.keys() if key in landmarks_idx ]
        errors = [train_results[key]["std"] for key in train_results.keys() if key in landmarks_idx]

        return slice_idx, landmarks_idx, x, predicted_scores, expected_scores, errors

    
    def plot_scores(self, vol_idx, save_path=False): 
        slice_idx, landmarks_idx, x, predicted_scores, expected_scores, errors = self.landmarks2score(vol_idx, 
                                                                                                 self.val_dataset, 
                                                                                                 self.train_lm_summary)
        plt.figure(figsize=(15, 8))
        plt.plot(np.array(x), np.array(predicted_scores),  linestyle="-", label="predicted slice score")
        expected_f = interpolate.interp1d(slice_idx, expected_scores, kind='linear')
        plt.errorbar(slice_idx, expected_scores, yerr=errors, marker="x", color="orange",
                     linestyle="", label="expected slice score (from training set)")
        plt.plot(np.array(x), expected_f(np.array(x)), color="orange", linestyle="--")

        for i, landmark in enumerate(landmarks_idx):
            plt.annotate(landmark, xy=(slice_idx[i]+0.1, expected_scores[i] + np.max(expected_scores)*0.05))    
        ...
        plt.ylabel("slice score", fontsize=14)
        if save_path: plt.savefig(save_path + f"predicted-vs-estiamted-slice-score-vol{vol_idx}.png", dpi=300, transparent=False, bbox_inches='tight')
        plt.show()
    ...
    def plot_pred2expected_scores(self, save_path=False, ids=np.arange(0, 65)):
        ...
        for i in tqdm(ids): #len(val_dataset)
            slice_idx, landmarks_idx, x, predicted_scores, expected_scores, errors = self.landmarks2score(i,
                                                                                                          self.val_dataset,
                                                                                                          self.train_lm_summary)
            if (len(np.unique(slice_idx)) != len(slice_idx)) or len(slice_idx) < 4: continue



            expected_f = interpolate.interp1d(slice_idx, expected_scores, kind='linear')
            label = str(i) + "_" + self.val_dataset.landmarks[i]["filename"][0:8] + \
                    self.val_dataset.landmarks[i]["filename"][-10:].replace(".npy", "").replace("0","")
        ...
        if save_path: plt.savefig(save_path + f"predicted-vs-estiamted-slice-score-multiple-volumes.png", dpi=300, transparent=False, bbox_inches='tight')
        plt.show()
    ...
    def print_summary(self): 
        print("Model summary\n*******************************")
        print(f"Landmark metric for validation set:\t{self.val_metrics[0]['test_landmark_metric_mean']:<1.4f}")
        print(f"Landmark metric for train set:     \t{self.train_metrics[0]['test_landmark_metric_mean']:<1.4f}")
        print(f"\nValidation accuracy:             \t{self.val_acc*100:<1.2f}% +- {self.val_std*100:<1.2f}%")
        print(f"Mean relative deviation (in 1e-6): \t{self.rmse*1e6:1.1f} +- {self.rmse_std*1e6:1.1f}")
    ...
    def rmse_for_volume(self, vol_idx): 
        ...
        """
        slice_idx, landmarks_idx, x, y_estimated, expected_scores, errors = self.landmarks2score(vol_idx, 
                                                                                             self.val_dataset, 
                                                                                             self.train_lm_summary)
        expected_f = interpolate.interp1d(slice_idx, expected_scores, kind='linear')
