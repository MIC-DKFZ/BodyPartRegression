...
def plot_dicomexamined_distribution(df, column="tag", count_column="count", fontsize=20): 
...
def plot_scores(filepath, fontsize=18): 
    ...
    try: 
        ...
        landmarks = dflandmarks[(dflandmarks["mean"] > min_score) & (dflandmarks["mean"] < max_score)]
...
def plot_volume(filepath, min_index=0, max_index=np.nan, title=""): 
    ...
    plt.imshow(X[:, 250, min_index:max_index].T, 
               origin="lower", 
               cmap="gray", 
               vmax=1500, 
               vmin=-1000, 
...
def plot_tailored_volumes_interactive(dfchests, start_score, end_score, json_base_path,  
                                     nifti_base_path): 
    def plotit(idx):
        ...
        _ = plot_volume(os.path.join(nifti_base_path, filename), title="Original Volume")
        _ = plot_volume(os.path.join(nifti_base_path, filename), min_index, max_index, title="Tailored Volume")
