...
def convert_ct_lymph_nodes_to_nifti(dicom_path, nifti_path): 
    ...
    dicom_dirs = [mydir for mydir in np.sort(os.listdir(dicom_path)) if mydir.startswith(("ABD", "MED"))]
    ...
    for dicom_dir in tqdm(dicom_dirs): 
        ...
        if os.path.exists(ofilepath): continue
...
def nifti2npy(nifti_path, npy_path):
    ...
    n2n = Nifti2Npy(target_pixel_spacing=7, # in mm/pixel
                    ...
                    rescale_min=-1) # rescale min value
...
def update_meta_data(landmark_filepath, meta_data_filepath): 
    ...
    """
    df_landmarks = pd.read_excel(landmark_filepath, 
                                sheet_name="database")
    df_meta_data = pd.read_excel(meta_data_filepath, index_col=0)

    train_filenames = [f + ".npy" for f in df_landmarks.loc[df_landmarks.train == 1, "filename"]]
    val_filenames = [f + ".npy" for f in df_landmarks.loc[df_landmarks.val == 1, "filename"]]
    test_filenames = [f + ".npy" for f in df_landmarks.loc[df_landmarks.test == 1, "filename"]]
