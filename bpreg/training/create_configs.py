...
def get_basic_config(mode="cluster", size=128):
    ...
    gaussian_noise = GaussNoise(
        std_min=0, std_max=0.04, min_value=-1, max_value=1, p=0.5
    )  # equivalent to max gaussian std noise of 50 HU
    shift_hu = ShiftHU(
        shift_limit=0.08, min_value=-1, max_value=1, p=0.5
    )  # equivalent to max shift of 100 HU
    ...
    gaussian_blure = A.GaussianBlur(
        blur_limit=(3, 7), sigma_limit=0.5, always_apply=False, p=0.5
    )

    custom_transform = transforms.Compose(
        [gaussian_noise, shift_hu, scale_hu, add_frame]
    )
    albumentation_transform = A.Compose(
        [flip, transpose, shift_scale_rotate, gaussian_blure]
    )

    custom_transform_params = {}
    for i in range(len(custom_transform.__dict__["transforms"])):
        custom_transform_params[
            custom_transform.__dict__["transforms"][i].__class__
        ] = custom_transform.__dict__["transforms"][i].__dict__

    albumentation_transforms_params = albumentation_transform.__dict__[
        "transforms"
    ].transforms
    ...
    config["accumulate_grad_batches"] = int(
...
if __name__ == "__main__":
    ...
    for idx, data in experiments.items():
        ...
        save_path_folder = (
            "../../src/configs/" + mode + "/" + config["model_name"] + "/"
        )
